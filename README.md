...
| field | when | meaning |
|---|---|---|
| `filterable` | query-time | up to 8 declared filter fields, each a logical `name` mapped to a payload dot-`path` (path bound 256 runes); names are the vocabulary queries may then filter by (`"filter":{"name":"value"}`). Declaring fields changes the resolved-config digest (and so version stamps) but never the index — no rebuild |
